...
function initLiveLots() {
  ...
  // Try loading scraped lots data first, then fall back to sample data
  fetchScrapedLots();
...
async function fetchScrapedLots() {
  const status = document.getElementById('llStatus');
  const notice = document.getElementById('llNotice');
  if (status) status.textContent = 'Loading live lots...';
  try {
    // Fetch the scraped lots JSON file (generated by scraper.php cron job)
    const cacheBust = Math.floor(Date.now() / 300000); // refresh every 5 min
    const res = await fetch(`lots-data.json?v=${cacheBust}`);
    if (!res.ok) throw new Error('lots-data.json not found');
    const data = await res.json();
    const lotsArray = data.lots || data.data || data;
    if (!Array.isArray(lotsArray) || lotsArray.length === 0) throw new Error('No lots in data');
    liveLots = lotsArray.map((lot, i) => ({
      id: lot.id || i + 1,
      title: lot.title || '',
      address: lot.address || lot.title || '',
      region: lot.region || 'National',
      type: lot.property_type || 'residential',
      guidePrice: lot.guide_price || lot.guide_price_low || 0,
      auctionDate: lot.auction_date || null,
      imageUrl: lot.image_url || null,
    ...
    if (data.generated_at) {
      const updatedEl = document.getElementById('llUpdated');
      if (updatedEl) {
        const d = new Date(data.generated_at);
        updatedEl.textContent = `Last updated: ${d.toLocaleDateString('en-GB')} ${d.toLocaleTimeString('en-GB', {hour:'2-digit', minute:'2-digit'})}`;
        updatedEl.style.display = 'block';
      }
    }
    renderLiveLots();
  } catch (err) {
    console.log('lots-data.json unavailable, using sample data:', err.message);
    loadFallbackLots();
  }
}
